...
class WatchStreamService @Autowired constructor(private val mongoTemplate: MongoTemplate) {
    companion object {
        ...
        private const val DISPOSABLE_EMAIL_DOMAINS = "disposable_email_domains"
        private const val INSERT = "insert"
        private const val UPDATE = "update"
        private const val OP_TYPE = "operationType"
    }

    fun watchEmailDomainAdditionStream() {
        val database = mongoTemplate.mongoDatabaseFactory.getMongoDatabase(DATABASE)
        val collection = database.getCollection(DISPOSABLE_EMAIL_DOMAINS)
        val operations: List<String> = arrayListOf(INSERT, UPDATE)
        val pipeline: MutableList<Bson> = mutableListOf(
                Aggregates.match(Filters.`in`(OP_TYPE, operations)))
